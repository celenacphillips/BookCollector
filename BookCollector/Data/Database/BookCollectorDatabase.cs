...
namespace BookCollector.Data.Database
{
    public partial class BookCollectorDatabase : BaseViewModel
    {
        ...
        public async Task CleanupBooks()
        {
            ...
            var allBooks = await this.database.Table<BookDatabaseModel>()
                .ToListAsync();
        ...
        public async Task CleanupWishlistBooks()
        {
            await this.Init();

            var allBooks = await this.database.Table<WishlistBookDatabaseModel>()
                .ToListAsync();
        ...
        public async Task CleanupAuthors()
        {
            await this.Init();

            var allAuthors = await this.database.Table<AuthorDatabaseModel>()
                .ToListAsync();

            foreach (var author in allAuthors)
            {
                ConvertTo<AuthorModel>(author).SetTotalBooks(true);
                ConvertTo<AuthorModel>(author).SetTotalCostOfBooks(true);
            }
        }
        ...
        public async Task CleanupCollections()
        {
            ...
            var allCollections = await this.database.Table<CollectionDatabaseModel>()
                .ToListAsync();

            foreach (var collection in allCollections)
            {
                ConvertTo<CollectionModel>(collection).SetTotalBooks(true);
                ConvertTo<CollectionModel>(collection).SetTotalCostOfBooks(true);
            }
        }
        ...
        public async Task CleanupGenres()
        {
            ...
            var allGenres = await this.database.Table<GenreDatabaseModel>()
                .ToListAsync();

            foreach (var genre in allGenres)
            {
                ConvertTo<GenreModel>(genre).SetTotalBooks(true);
                ConvertTo<GenreModel>(genre).SetTotalCostOfBooks(true);
            }
        }
        ...
        public async Task CleanupSeries()
        {
            await this.Init();

            var allSeries = await this.database.Table<SeriesDatabaseModel>()
                .ToListAsync();

            foreach (var series in allSeries)
            {
                ConvertTo<SeriesModel>(series).SetTotalBooks(true);
                ConvertTo<SeriesModel>(series).SetTotalCostOfBooks(true);
            }
        }
        ...
        public async Task CleanupLocations()
        {
            await this.Init();

            var allLocations = await this.database.Table<LocationDatabaseModel>()
                .ToListAsync();

            foreach (var location in allLocations)
            {
                ConvertTo<LocationModel>(location).SetTotalBooks(true);
                ConvertTo<LocationModel>(location).SetTotalCostOfBooks(true);
            }
        }
